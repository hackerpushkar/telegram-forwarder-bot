...
def is_mongo_enabled() -> bool:
    """Returns True if a valid MongoDB URI is provided in configuration."""
    return bool(config.MONGO_URI and config.MONGO_URI.strip())

def get_mongo_client():
    """Lazily initializes and returns the AsyncIOMotorClient."""
    global _mongo_client
    if _mongo_client is None:
        from motor.motor_asyncio import AsyncIOMotorClient
        _mongo_client = AsyncIOMotorClient(config.MONGO_URI.strip())
    return _mongo_client

def get_mongo_db():
    """Returns the configured Mongo database instance."""
    client = get_mongo_client()
    return client[config.MONGO_DB_NAME]

async def get_next_sequence(sequence_name: str) -> int:
    """Atomically increments and returns the next integer ID sequence in MongoDB."""
...
async def init_db():
    """Initialize database tables/collections and indexes for either MongoDB or SQLite."""
    if is_mongo_enabled():
        logger.info("Initializing MongoDB connection to database '%s'...", config.MONGO_DB_NAME)
        db = get_mongo_db()
        try:
            # Create indexes
            await db.routes.create_index("id", unique=True)
            await db.routes.create_index([("source_chat_id", 1), ("is_active", 1)])
            await db.routes.create_index("user_id")

            await db.route_filters.create_index("route_id", unique=True)
            await db.route_customizations.create_index("route_id", unique=True)
            await db.route_replacements.create_index("id", unique=True)
            await db.route_replacements.create_index("route_id")

            await db.userbot_auth.create_index("id", unique=True)
            await db.bot_stats.create_index("key", unique=True)

            # Initialize default stats
            for stat_key in ["total_forwarded", "total_errors", "total_filtered", "userbot_forwarded"]:
                await db.bot_stats.update_one(
                    {"key": stat_key},
                    {"$setOnInsert": {"key": stat_key, "value": 0}},
                    upsert=True
                )

            logger.info("MongoDB database '%s' initialized and indexes verified successfully.", config.MONGO_DB_NAME)
        ...
        async with get_db() as db:
            ...
            await db.commit()
            logger.info("SQLite Database initialized successfully at %s", config.DB_PATH)
